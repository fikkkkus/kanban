    ...
    <main class="relative min-h-screen overflow-hidden bg-gradient-to-br from-indigo-950 via-violet-900 to-fuchsia-900 px-4 text-slate-900">
        ...
        <div class="relative z-10 mx-auto flex min-h-screen w-full max-w-md items-center">
            <div class="w-full rounded-2xl border border-white/25 bg-white/90 p-6 text-slate-900 shadow-[0_24px_60px_rgba(0,0,0,0.25)] backdrop-blur-xl">
                <div class="mb-5">
                    <h1 class="mt-3 text-2xl font-semibold text-slate-900">Создать аккаунт</h1>
                </div>

                <form class="space-y-4" @submit.prevent="submit">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-900">Имя</label>
                        <input
                            v-model="form.name"
                            required
                        ...
                        />
                        <p v-if="auth.state.errors.name" class="mt-1 text-sm text-red-600">
                            {{ auth.state.errors.name[0] }}
                        </p>
                    </div>
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-900">Подтвердите пароль</label>
                        <input
                ...
                <p class="mt-4 text-sm text-slate-600">
                    Уже есть аккаунт?
                    <a href="/kanban/login" class="font-medium text-violet-700">Войти</a>
                </p>
            </div>
        </div>
    </main>
